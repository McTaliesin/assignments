...
this.newCoin=this.newCoin.bind(this);
  ...
  componentDidMount(){
    axios.get("https://api.coinmarketcap.com/v1/ticker/?start=0&limit=10").then(response => {
 ...
 newCoin(){
  ...
  render() {
     ...
     const coins = this.state.coins.map(coin => {
       return <div>The name is: {coin.name}, ---The current price in USD is---: {coin.price_usd}</div>
     })
  return (
