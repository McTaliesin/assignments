...
/***/ function(module, exports, __webpack_require__) {
	...
	exports.dispatchLogger = exports.connect = exports.Provider = exports.compose = exports.applyMiddleware = exports.bindActionCreators = exports.combineReducers = exports.createStore = exports.seeduxInit = undefined;

	var _seeduxLogger = __webpack_require__(3);

	var _seeduxLogger2 = _interopRequireDefault(_seeduxLogger);
	...
	*/
	...
	function seeduxInit(store) {
	  ...
	  (0, _bindActionCreators2['default'])(actions, store.dispatch);
	  store.dispatch({ type: 'SEEDUX_INIT' });
	  document.addEventListener('dispatchAction', function (e) {
	    store.dispatch(e.detail.action);
	  }, false);
	}

	exports.seeduxInit = seeduxInit;
	...
	exports.dispatchLogger = _seeduxLogger2['default'];
	exports['default'] = _seeduxLogger2['default'];
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _deepDiff = __webpack_require__(4);
	...
	*/
	...
	var listenerFlag = false; /* global document */


	var dispatchLogger = function dispatchLogger(_ref) {
	  ...
	  return function (next) {
	    return function (action) {
	      ...
	      */
	      ...
	      var modifiedAction = next(action);
	      ...
	      if (listenerFlag === false) {
	        listenerFlag = true;
	        document.addEventListener('seeduxRestore', function (e) {
	          next({
	            type: 'seedux_RESTORE',
	      ...
	      var diffs = (0, _deepDiff.diff)(oldState, newState) || [];
	      var newHistoryEntry = {
...
/***/ function(module, exports, __webpack_require__) {
	...
	function createStore(reducer, preloadedState, enhancer) {
	  ...
	  dispatch({ type: ActionTypes.INIT });
	  // SEEDUX
	  // snooping on the initial state, and sending it to content script by creating and emitting
	  // a special case of our 'actionDispatched' DOM CustomEvent
	  var initialState = getState();
	  var newHistoryEntry = {
	    originalAction: { type: '@@INIT' },
	    modifiedAction: { type: '@@INIT' },
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _seeduxExtractor = __webpack_require__(23);
	...
	function combineReducers(reducers) {
	  ...
	  if (reducers) {
	    reducerKeys.forEach(function (key) {
	      if (reducers[key]) {
	        var reducerName = reducers[key].name === '_default' ? key : reducers[key].name || key;
	        var reducerDef = reducers[key].toString();
	        seeduxObj[reducerName] = reducerDef;
	      }
	    });
	  }
	  (0, _seeduxExtractor.reducersExtractor)(seeduxObj);
	  // end seedux
	  ...
	  return function combination() {
	    ...
	    // seedux
	    if (action && action.type === 'seedux_RESTORE') {
	      // console.log('RESTORING...', action.restoreState);
	      var theNewPresent = action.restoreState;
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _require = __webpack_require__(24),
	    assembleUINodes = _require.assembleUINodes,
	    assembleReducerNodes = _require.assembleReducerNodes,
	    assembleActionCreatorNodes = _require.assembleActionCreatorNodes,
	    resetUIHeadNode = _require.resetUIHeadNode;
	...
	*/
	...
	function uiExtractor(ui) {
	  var defaultMapStateToProps = 'function defaultMapStateToProps(state){return{};}';
	  var defaultMapDispatchToProps = 'function defaultMapDispatchToProps(dispatch){return{dispatch:dispatch};}';
	  var uiName = Object.keys(ui)[0];
	  var uiPropNames = ui[uiName];
	  var mapStateToPropsDef = void 0,
	      mapStateToPropsSplitAtKeys = void 0,
	      mapDispatchToPropsDef = void 0,
	      mapDispatchToPropsSplit = void 0,
	      uiStateKeys = void 0,
	      uiStatePropNames = void 0,
	      uiDispatchPropNames = void 0,
	      uiActionCreators = void 0;
	  var uiMapState = {};
	  var uiMapDispatch = {};
	  uiMapState[uiName] = {};
	  ...
	  if (uiPropNames) {
	    if (!mapStateToPropsDef && !mapDispatchToPropsDef) {
	      var splitPropNames = uiPropNames.split('|');
	      ...
	      if (mapStateToPropsDef.includes('state.')) {
	        uiStateKeys = uiStateKeys.split('state.').slice(1).map(function (subStr) {
	          return subStr.slice(0, subStr.lastIndexOf('.'));
	        }).map(function (subStr) {
	          return subStr.replace(/[^\w.]/gi, '');
	        });
	      } else if (mapStateToPropsDef.includes('store.')) {
	        uiStateKeys = uiStateKeys.split('store.').slice(1).map(function (subStr) {
	          return subStr.slice(0, subStr.lastIndexOf('.'));
	        }).map(function (subStr) {
	          return subStr.replace(/[^\w.]/gi, '');
	        });
	      }
	      uiStatePropNames = mapStateToPropsDef.split(':').map(function (subStr) {
	        return subStr.slice(-1 * (subStr.length - 1 - subStr.lastIndexOf(' ')));
	      }).filter(function (subStr) {
	        return (/[\w*]/gi.test(subStr)
	        );
	      });
	      uiDispatchPropNames = mapDispatchToPropsDef.split(':').map(function (subStr) {
	        return subStr.slice(-1 * (subStr.length - 1 - subStr.lastIndexOf(' ')));
	      }).filter(function (subStr) {
	        return (/[\w*]/gi.test(subStr)
	        );
	      });
	      uiActionCreators = uiActionCreators.split('dispatch(').slice(1).map(function (subStr) {
	        return subStr.slice(subStr.indexOf('.') + 1, subStr.indexOf(')'));
	      }).map(function (subStr) {
	        return subStr.replace(/\(\w+/gi, '');
	      });

	      uiStatePropNames.forEach(function (prop) {
	        uiMapState[uiName][prop] = [];
	        uiStateKeys.forEach(function (key) {
	          uiMapState[uiName][prop].push(key);
	        });
	      });

	      uiDispatchPropNames.forEach(function (prop) {
	        uiMapDispatch[uiName][prop] = [];
	        uiActionCreators.forEach(function (ac) {
	          mapDispatchToPropsSplit.forEach(function (def) {
	    ...
	    uiPropNames = uiPropNames.split(':').map(function (subStr) {
	      return subStr.slice(-1 * (subStr.length - 1 - subStr.lastIndexOf(' ')));
	    }).filter(function (subStr) {
	      return (/[\w*][^\s+]/gi.test(subStr)
	      );
	    });
	  }
	...
	*/
	...
	function reducersExtractor(reducers) {
	  ...
	  var strReducers = JSON.stringify(reducers);
	  var reducerCases = void 0;
	  if (strReducers.includes('switch')) {
	    reducerCases = strReducers.split('case').slice(1).map(function (cases) {
	      return cases.replace(/[^\w*:.]/g, '');
	    }).map(function (cases) {
	      return cases = cases.slice(0, cases.indexOf(':'));
	    });
	  } else if (strReducers.includes('action.type') && strReducers.includes('if')) {
	    reducerCases = strReducers.split('action.type').slice(1).map(function (subStr) {
	      return subStr.replace(/[^\w\\.)]/g, '');
	    }).map(function (subStr) {
	  ...
	  if (/\./.test(reducerCases)) {
	    reducerCases = reducerCases.map(function (cases) {
	      return cases = cases.slice(cases.indexOf('.') + 1);
	    });
	  }
	...
	*/
	...
	function actionCreatorsExtractor(actionCreators) {
	  var actionCreatorNames = Object.keys(actionCreators);
	  var strActionCreators = JSON.stringify(actionCreators);
	  var actionTypes = void 0;

	  actionTypes = strActionCreators.split('type:').slice(1).map(function (subStr) {
	    return subStr.slice(1, subStr.indexOf('\\')).replace(/[^\w*.,]/g, '');
	  });
	  if (/\./.test(actionTypes)) {
	    actionTypes = actionTypes.map(function (subStr) {
	      return subStr.slice(subStr.indexOf('.') + 1);
	    });
	  }
	  if (/\,/.test(actionTypes)) {
	    actionTypes = actionTypes.map(function (subStr) {
	...
	module.exports = { uiExtractor: uiExtractor, actionCreatorsExtractor: actionCreatorsExtractor, reducersExtractor: reducersExtractor };
...
/***/ function(module, exports) {
	...
	var uiHeadNode = new Node('Containers');
	var acHeadNode = new Node('Action Creators');
	uiHeadNode.children = [];
	acHeadNode.children = [];
	var actionMap = {};
	...
	function assembleUINodes(uiName) {
	  var uiPropNames = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : [];
	  var uiMapState = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : [];
	  var uiMapDispatch = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : [];

	  var uiNameNode = new Node(uiName);
	  if (uiPropNames) {
	    uiNameNode.children = [];
	    uiPropNames.forEach(function (p) {
	      if (!/[^\w]/gi.test(p)) {
	...
	function assembleReducerNodes(reducers, reducerNames) {
	  var reducerCases = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : [];

	  var headNode = new Node('Reducers');
	  headNode.children = [];
	  reducerNames.filter(function (name) {
	    return name !== 'undefined';
	  }).forEach(function (name, i) {
	    var reducerNode = new Node(name);
	    reducerNode.children = [];
	    if (reducerCases[i]) {
	      reducerCases.filter(function (c) {
	        return c !== 'undefined';
	      }).forEach(function (c) {
	...
	function assembleActionCreatorNodes(actionCreatorNames, actionTypes) {
	  actionCreatorNames.forEach(function (ac, i) {
	    var actionCreatorNode = new Node(ac);
	...
	module.exports = { assembleUINodes: assembleUINodes, assembleReducerNodes: assembleReducerNodes, assembleActionCreatorNodes: assembleActionCreatorNodes, resetUIHeadNode: resetUIHeadNode, resetACHeadNode: resetACHeadNode, Node: Node };
...
/***/ function(module, exports, __webpack_require__) {
	...
	function bindActionCreators(actionCreators, dispatch) {
	  ...
	  if (typeof actionCreators === 'function') {
	    ...
	    (0, _seeduxExtractor.actionCreatorsExtractor)(seeduxObj);
	  ...
	  if (keys.length) {
	    keys.forEach(function (ac) {
	      if (actionCreators[ac]) {
	        seeduxObj[ac] = actionCreators[ac].toString();
	      };
	    });
	  }
	  (0, _seeduxExtractor.actionCreatorsExtractor)(seeduxObj);
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _seeduxExtractor = __webpack_require__(214);
	...
	function connect(mapStateToProps, mapDispatchToProps, mergeProps) {
	  ...
	  // seedux
	  var mappedStateString = mapState.toString();
	  var mappedDispatchString = mapDispatch.toString();
	  var mappedPropsString = mappedStateString + '|' + mappedDispatchString;

	  return function wrapWithConnect(WrappedComponent) {

	    var UI = WrappedComponent.name;
	    (0, _seeduxExtractor.uiExtractor)(_defineProperty({}, UI, mappedPropsString));
	    // end seedux
